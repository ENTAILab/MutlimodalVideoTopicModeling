...
def summarize_topics_extractively(
    ...
    for topic_id, sents in grouped.items():
        ...
        try:
            ...
            # Fallback for short/noisy text where stop-word filtering prunes all terms.
            token_counts = Counter(token for sent in sents for token in sent.lower().split())
            scores = np.asarray([
                float(sum(token_counts.get(token, 0) for token in sent.lower().split()))
                for sent in sents
            ])
            idx = np.argsort(scores)[::-1][:max_sentences_per_topic]
